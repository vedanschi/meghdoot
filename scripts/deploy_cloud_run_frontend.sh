...
API_ORIGIN="${API_ORIGIN:-https://megdhoot-backend-slg6ucjxfq-uk.a.run.app}"
...
gcloud run deploy "${FRONTEND_SERVICE_NAME}" \
  ...
  --max-instances 3 \
  --port 8080 \
  --set-env-vars "PORT=8080,API_ORIGIN=${API_ORIGIN},NEXT_PUBLIC_GCS_BUCKET=${NEXT_PUBLIC_GCS_BUCKET},NEXT_PUBLIC_FORECAST_PREFIX=${NEXT_PUBLIC_FORECAST_PREFIX},NEXT_PUBLIC_REFRESH_MS=${NEXT_PUBLIC_REFRESH_MS},NEXT_PUBLIC_STALE_AFTER_MINUTES=${NEXT_PUBLIC_STALE_AFTER_MINUTES}"
